...
#' @importFrom RcisTarget getNumColsInDB addMotifAnnotation addSignificantGenes
#' @importFrom data.table rbindlist is.data.table
#' @examples
#' print("test")
#' @export
runSCENIC_2_createRegulons <- function(scenicOptions, minGenes=20, coexMethod=NULL)
{
  ...
  if(any(!loadAttempt)) stop("It is not possible to load the following databses: \n",
                             paste(scenicOptions@settings$"dbs"[which(!loadAttempt)], collapse="\n"))
